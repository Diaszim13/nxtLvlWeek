...
async show(req:Request, res:Response){
    ...
    if(!point) {
        return res.status(400).json({ message: 'Point not found.' });
         ...
         /*SELECT * FROM items JOIN point_items ON items_id = point items.item.id WHERE point_items.point_id = {id}*/
    ...
    async  create (req:Request,res:Response)  {
        ...
        const trx = await knex.transaction();//se der erro em uma requisição ao utra para 
        ...
        return res.json({ 
            id: point_id,
            ...point,// '...' pega todos os dados do objeto point
